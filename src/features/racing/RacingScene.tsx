...
import { PerspectiveCamera, useGLTF, useAnimations, Html } from '@react-three/drei';
...
function PlayerModel({ entity }: { entity: RacingEntity }) {
    ...
    useEffect(() => {
        ...
        let actionKey = Object.keys(actions).find(k => k.toLowerCase().includes(animName.toLowerCase()));
        
        if (!actionKey && animName === 'run') actionKey = Object.keys(actions).find(k => k.toLowerCase().includes('walk'));
        if (!actionKey) actionKey = Object.keys(actions)[0];
...
function RiverBanks() {
    ...
    useFrame((_, delta) => {
        ...
        const scrollSpeed = useRacingStore.getState().status === 'playing' ? RACING_CONFIG.BASE_SCROLL_SPEED : 0;
...
function Player() {
  ...
  return <>{entities.map((e, i) => <PlayerModel key={e.id || i} entity={e} />)}</>;
}

function Obstacles() {
    const entities = useArchetype(racingWorld.with('obstacle', 'position', 'model'));
    return <>{entities.map((e, i) => <SimpleModel key={e.id || i} entity={e} />)}</>;
}
    ...
    return (
        <Html fullscreen>
            <div style={{
                ...
                {/* HUD */}
                {status === 'playing' && (
                    <div style={{ padding: '20px', display: 'flex', justifyContent: 'space-between', textShadow: '2px 2px 4px rgba(0,0,0,0.5)' }}>
                        <div>
                            <div style={{ fontSize: '32px', fontWeight: 'bold', color: '#FFD700' }}>Score: {score}</div>
                            <div style={{ fontSize: '20px' }}>Distance: {Math.floor(distance)}m</div>
                        </div>
                ...
                {(status === 'game_over' || status === 'menu') && (
                    <div style={{
                        ...
                        <div style={{
                            ...
                            <h1 style={{ color: status === 'game_over' ? '#ff4444' : '#00aaff', fontSize: '48px', margin: '0 0 20px 0' }}>
                            ...
                            {status === 'game_over' && (
                                <div style={{ marginBottom: '30px' }}>
                                    ...
                                    <p style={{ fontSize: '20px', color: '#aaa' }}>Distance: {Math.floor(distance)}m</p>
                                </div>
                            )}
...
function RacingSystems() {
    ...
    useFrame((_state, delta) => {
        ...
        if (spawnTimer.current > (RACING_CONFIG.SPAWN_INTERVAL * (RACING_CONFIG.BASE_SCROLL_SPEED / currentSpeed))) {
            ...
            if (rand > 0.4) {
                const rockVariants = [ASSET_URLS.MODELS.ROCK_RIVER, ASSET_URLS.MODELS.ROCK_MOSSY, ASSET_URLS.MODELS.ROCK_CRACKED];
            ...
            for (const obs of racingWorld.with('obstacle', 'position')) {
                ...
                if (dz < 1.0 && dx < 0.7) {
                    ...
                    if (player.animation) {
                        ...
                        setTimeout(() => { if (player.animation) player.animation.current = 'run'; }, 500);
                    }
                }
            }
    ...
    useEffect(() => {
        const handleKey = (e: KeyboardEvent) => {
             ...
             if ((e.key === 'ArrowLeft' || e.key === 'a') && newLane > -1) newLane--;
        ...
        const onTouchEnd = (e: TouchEvent) => {
            ...
            if (Math.abs(deltaX) > Math.abs(deltaY) && Math.abs(deltaX) > 30) {
                ...
                if (player && player.lane != null) {
                    let newLane = player.lane;
                    if (deltaX > 0 && newLane < 1) newLane++;
                    else if (deltaX < 0 && newLane > -1) newLane--;
                    if (newLane !== player.lane) {
                        player.lane = newLane;
                        player.position.x = RACING_CONFIG.LANES[newLane + 1];
                    }
